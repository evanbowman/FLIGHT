...
std::vector<std::weak_ptr<Solid>> & Sector::GetSolids() {
    return m_solids;
}

std::list<std::pair<std::shared_ptr<Solid>, std::shared_ptr<Solid>>> Sector::GetPairs() const {
...
void CollisionManager::UpdateSector(const std::pair<int, int> & coord, Sector & sector) {
    for (auto it = sector.GetSolids().begin(); it != sector.GetSolids().end();) {
	...
	if (pair.first->GetAABB().Intersects(pair.second->GetAABB())) {
	    ...
	    // Idea: if AABBs intersect, get set of component OBBs and test those
...
void CollisionManager::AddSolid(std::shared_ptr<Solid> solid) {
    m_sectorTree[CalcTargetSector(solid->GetPosition())].GetSolids().push_back(solid);
}
