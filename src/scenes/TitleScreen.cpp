...
void TitleScreen::UpdateLogic(const Time dt) {
    ...
    case State::ShowTitle:
        ...
        if (m_transitionTimer < ENTRY_TRANSITION_TIME) {
            ...
            float diff = glm::smoothstep(0.f, (float)ENTRY_TRANSITION_TIME,
                                         (float)m_transitionTimer);
            const auto & windowSize = GetGame().GetWindowSize();
            m_titleText.SetPosition(
                {windowSize.x / 2 - m_titleText.GetSize().x / 2.f,
                 windowSize.y - 2.5 * m_titleText.GetSize().y * diff, 0.f});
            m_startText.SetPosition(
                {windowSize.x / 2 - m_startText.GetSize().x / 2.f,
                 4 * m_startText.GetSize().y * diff, 0.f});
        }
        ...
        if (GetGame().GetInput().buttonSet->WeaponPressed()) {
            ...
            m_timer = 0;
            m_transitionTimer = 0;
            dynamic_cast<ElasticCamera &>(GetGame().GetCamera())
                .SetPosition({0, 0.1, -3});
        }
        break;
