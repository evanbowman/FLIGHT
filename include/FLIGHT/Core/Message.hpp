...
using Message =
    mapbox::util::variant<Collision, Death, TerrainCollision, PickedUpCoin>;

using MessageRef = std::unique_ptr<Message, std::function<void(Message *)>>;

template <typename T, typename... Args> MessageRef NewMessage(Args &&... args) {
    return {Singleton<MemPool<Message>>::Instance().New(
...
public:
    ...
    template <typename F> void PollMessages(F && callback) {
